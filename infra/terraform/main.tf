...
# No NAT Gateway — all ECS tasks run in public subnets with public IP
# No private subnets needed — saves ~$36/month
...
resource "aws_security_group" "internal" {
  ...
  description = "Internal service access (Redis, Qdrant)"
  vpc_id      = aws_vpc.this.id
  ...
  ingress {
    ...
    to_port         = 2049
    protocol        = "tcp"
    description     = "EFS mount for Qdrant and shared uploads"
    security_groups = [aws_security_group.api.id, aws_security_group.worker.id]
    self            = true
  }
  ...
  tags = merge(local.tags, { Name = "${local.name_prefix}-internal-sg" })
}
